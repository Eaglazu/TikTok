...
const DEFAULT_RPM = "€0.51"; // Définir la valeur par défaut pour le RPM
...
const Dash1 = () => {
  ...
  const getAverage7Days = useStore((state) => state.getAverage7Days);
  const getAverage30Days = useStore((state) => state.getAverage30Days);
  ...
  const [average7Days, setAverage7Days] = useState(getAverage7Days());
  const [average30Days, setAverage30Days] = useState(getAverage30Days());

  useEffect(() => {
    setAverage7Days(getAverage7Days());
  }, [dataPoints7Days, getAverage7Days]);

  useEffect(() => {
    setAverage30Days(getAverage30Days());
  }, [dataPoints30Days, getAverage30Days]);
  ...
  const [previous30DaysData, setPrevious30DaysData] = useState([]);
  ...
  useEffect(() => {
    ...
    if (!isNaN(newEntry) && newEntry !== lastValue.current) {
      ...
      updateDataPoints7Days(newEntry); // Mettre à jour les points de données pour les 7 jours avec la nouvelle valeur
    }
  ...
  const updateDataPoints7Days = useCallback(
    (newEntry) => {
      setDataPoints7Days((prevData) => {
        const removedData =
          prevData.length >= MAX_DATA_POINTS ? prevData[0] : null;
        const newData =
        ...
        if (removedData !== null) {
          setPrevious7DaysData((prev) => [...prev, removedData]);
          console.log("Removed from 7 days:", removedData);
        }
      ...
      saveData(newDataPoints7Days, dataPoints30Days);
    },
    [dataPoints30Days, saveData]
  );
  ...
  const updateDataPoints30Days = useCallback(() => {
    ...
    const removedData = dataPoints30Days.slice(-7);

    setDataPoints30DaysInStore(newData);
    setDateLabels30Days(newDateLabels);
    setPrevious30DaysData((prev) => [...prev, ...removedData]);
    removedData.forEach((item) => console.log("Removed from 30 days:", item));
    setViewsEligible30Days(formattedViews);
  }, [
    dataPoints30Days,
    dataPoints7Days,
    setPrevious30DaysData,
    setViewsEligible30Days,
    setDateLabels30Days,
  ]);
      ...
      <View style={styles.container}>
        ...
        <View style={styles.squareContainer}>
          ...
          <TouchableOpacity onPress={handleRPMPress} style={styles.square}>
            ...
            <Text style={styles.valueText}>{getRPM()}</Text>
          </TouchableOpacity>
        </View>
        <View style={styles.newsRectangle}>
          ...
          <View style={styles.textWithButton}>
            <Text style={styles.newsContent}>
              Les récompenses seront créditées
            </Text>
            <TouchableOpacity style={styles.balanceButton}>
              <Text style={styles.balanceButtonText}>Voir le solde</Text>
            </TouchableOpacity>
          </View>
          <Text style={styles.leftAlignedDate}> le {lastPaymentDate}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  balanceButton: {
    ...
    borderRadius: 10,
    marginTop: 5,
    marginLeft: 10, // Adjust this value to move the button to the right
  },
  balanceButtonHeader: {
    backgroundColor: "#f0f0f0",
    paddingVertical: 8,
