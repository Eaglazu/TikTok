...
const useStore = create(
  devtools((set) => ({
    ...
    getAverage7Days: () => {
      const sum = initialValues7Days.reduce((acc, val) => acc + val, 0);
      return (sum / initialValues7Days.length).toFixed(2);
    },
    getAverage30Days: () => {
      const sum = initialValues30Days.reduce((acc, val) => acc + val, 0);
      return (sum / initialValues30Days.length).toFixed(2);
    },
    validateInput: () =>
      set((state) => {
        const value = parseFloat(state.inputValue);
