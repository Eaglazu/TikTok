...
const CustomChart7days = ({ dataPoints = [], dateLabels = [] }) => {
  ...
  function updateNearestPoint(xLocation) {
    ...
    let minY = Number.MAX_SAFE_INTEGER; // Définir un nouveau tracker pour minY
  ...
  return (
    <View {...panResponder.panHandlers} style={{ flex: 1 }}>
      <Svg height={chartHeight + extraHeight + yOffset} width={chartFullWidth}>
        ...
        {dateLabels.map((date, index) => (
          <SvgText
            key={`date-${index}`}
            x={marginLeft + (chartWidth / (dateLabels.length - 1)) * index}
...
export default CustomChart7days;
